...
[![Skills](https://img.shields.io/badge/skills-50-yellow?style=flat-square)](#-스킬-인덱스)
...
| 스킬 | 설명 | 키워드 |
|------|------|--------|
| **strix** 🆕 | AI 기반 애플리케이션 보안 테스트 — Docker + LLM으로 로컬 디렉토리, GitHub 저장소, 스테이징 URL, 도메인 취약점 스캔 (authorized only) | `strix`, `ai pentest` |

### 📝 노트 & 지식 관리

| 스킬 | 설명 | 키워드 |
|------|------|--------|
| **obsidian-cli** 🆕 | 터미널 기반 Obsidian 자동화 — 실행 중인 Obsidian vault에 대해 명령 실행, 일일 노트, 검색, 태그, 작업, 개발자 명령 자동화 | `obsidian cli`, `obsidian uri` |
...
└── .unity-skills/
    ...
    ├── strix/                   ← AI 기반 애플리케이션 보안 스캐너 🆕
    ├── obsidian-cli/            ← 터미널 기반 Obsidian vault 자동화 🆕
    └── [35 domain skills]/
```
...
| 문서 | 설명 |
|------|------|
| [SKILLS-INDEX.md](SKILLS-INDEX.md) | 카테고리, 키워드, 빠른 선택 가이드가 포함된 49종 스킬 디렉토리 |
